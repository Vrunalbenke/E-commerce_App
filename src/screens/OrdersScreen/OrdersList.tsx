        ...
        {orderList.data.map((item,index)=>{
            return (
                <View
                    ...
                    >
                    <Image source={require('../../assets/images/orderList.jpg')} style={{height:100,width:100,resizeMode:'stretch',backgroundColor:'lightblue'}} />
                    <View>
                        <Text style={{fontSize:25,fontWeight:'700'}}>Order ID - #{item.id}</Text>
                    <Text style={{fontSize:18}}>Price: ₹{commafy(item.cost)}</Text>
                    </View>
                    ...
                    <Ionicons name="chevron-forward-sharp" size={30} style={{paddingBottom:10}}/>
                    </TouchableOpacity>
                </View>
            )
        })}
